<div class="flex">
    <div class="mr-2 flex-shrink-0">
        <a href="{{route('profile', $tweet->user) }}"><img width="50" src="{{ $tweet->user->getAvatar() }}"
                class="mr-2 rounded-full" alt=""></a>
    </div>
    <div>
        <h5 class="font-bold mb-2"> <a href="{{$tweet->user->profile()}}">{{ $tweet->user->name }}</a></h5>
        ...
        <div class="flex items-center">
            <form action="/tweets/{{$tweet->id}}/like" method="POST">
                @csrf
                <div
                    class="flex items-center  {{ $tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
                    ...
                    <button class="mt-1 text-xs ">{{$tweet->likes_count ?? '0'}}</button>
                </div>
            </form>
            ...
            <form action="/tweets/{{ $tweet->id }}/dislike" method="POST">
                @csrf
                <div
                    class="{{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}  ml-1 flex items-center">
                    <svg viewBox="0 0 20 20 " class="pt-1 mr-1 w-3 ">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g class="fill-current">
                                <path
                    ...
                    <button class="mt-1 text-xs">{{$tweet->dislikes_count ?? '0'}}</button>
                </div>
            </form>
        </div>
    </div>
